...
export default function CardGenerator({ title, description, generatorFn, allowBulk = true, children }) {
  ...
  return (
    ...
    >
      <div>
        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2 font-satoshi">
          {title}
          {isHovered && <span className="text-[10px] bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-1.5 py-0.5 rounded ml-auto font-sans">Ativo</span>}
        </h3>
        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">{description}</p>
      </div>

      <div className="flex items-center gap-2 mt-auto">
        <div className="flex-1 bg-slate-100 dark:bg-slate-900/50 border border-slate-300 dark:border-slate-700 rounded-lg p-3 font-mono text-lg text-emerald-600 dark:text-emerald-400 overflow-hidden text-ellipsis whitespace-nowrap">
          {value}
        </div>
