...
export default function Sidebar({ activeTab, setActiveTab, onOpenHistory }) {
  ...
  return (
    <aside className="w-64 glass-panel border-r border-slate-200 dark:border-slate-700 flex flex-col h-full hidden md:flex">
        <div className="flex items-center justify-center h-20 border-b border-slate-200 dark:border-white/10 relative">
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight flex items-center gap-2 font-satoshi lowercase">
            <span className="text-emerald-500 dark:text-emerald-400">simu</span>lae
          </h1>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-sm font-medium font-satoshi ${
                isActive 
                  ? 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 shadow-[inset_2px_0_0_0_#34d399]' 
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex flex-col gap-2">
        <button
          ...
          className="flex items-center gap-3 px-3 py-2 w-full rounded-md text-sm font-medium transition-all text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 dark:text-slate-400 dark:hover:text-emerald-400 dark:hover:bg-emerald-500/10"
        >
          <Clock size={18} />
          Histórico Local
        </button>
